# build-orchestrator Specification

## Purpose

`build-orchestrator` is the Build Bridge state-machine coordinator. `runBuild(opts)` looks up a `proposals` row, INSERTs a `builds` row, then drives Claude Code through plan → decompose → apply (×N changes) → validate (×N), persisting phase transitions to the row at every step. The orchestrator never throws on phase failure — it transitions `phase='failed'` with a tagged `failure_reason` (`plan_empty` / `decompose_empty` / `apply_failed_<id>` / `validation_failed_<id>` / `aborted`) and returns `{ status: 'failed', ... }`. On success of every change, the row lands at `phase='integrate'` and the result carries `status: 'ready_for_integration'` for the integration phase to pick up. Every phase runner is injectable via `PhaseRunner` so tests stub Claude without a real subprocess. `runApplyPhase` ships alongside as a thin wrapper that invokes the runner with `/opsx:apply <changeId>`.
