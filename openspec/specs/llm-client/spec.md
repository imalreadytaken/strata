...
`llm-client` wires the `LLMClient` seam (introduced by `triage`) to a real backend via `@mariozechner/pi-ai`. `PiAiLLMClient` translates `infer({ system, user })` into pi-ai's `complete(model, context, { apiKey })` and flattens the assistant's text content. `resolveLLMClient(config, opts)` is the runtime factory that picks `'pi-ai'` when `config.models.fast` is a valid `'<provider>/<modelId>'` AND an API key resolves (via env var or explicit override), and falls back to `'heuristic'` otherwise. The fallback path keeps triage usable when no LLM is configured / the env var is missing / the provider is unknown. AGENTS.md's "no API keys in config" stays honest: keys come from `process.env.*_API_KEY` via pi-ai's `getEnvApiKey`. Errors from pi-ai are wrapped as `STRATA_E_LLM_FAILED`; an empty assistant response becomes `STRATA_E_LLM_EMPTY_RESPONSE`.
