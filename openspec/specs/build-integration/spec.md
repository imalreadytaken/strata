...
`build-integration` is the Build Bridge tail: it picks up a `ready_for_integration` `BuildRunResult`, moves each freshly-validated capability from the build workdir into `<config.paths.capabilitiesDir>/<name>/v<N>/`, applies its migrations to the main DB, and writes the four bookkeeping rows (`capability_registry`, `capability_health`, `schema_evolutions`, plus updates to the originating `proposals` row and the `builds` row). Each capability integrates **transactionally**: a mid-flight DB failure rolls back the FS copy. Multi-capability partial success is preserved — capabilities that landed stay landed; the build's `failure_reason` names the one that broke. Programmer errors (build not in `ready_for_integration`, missing build row) throw; everything else returns a tagged `IntegrationResult`. The capability becomes visible to the runtime on next plugin boot (the loader walks the user dir at startup); an in-process hot-pickup hook is left for a follow-up change.
