# reextract-llm-strategies Specification

## Purpose

`reextract-llm-strategies` adds the two LLM-backed re-extract strategies: `reextract_raw_events` (uses the linked `raw_events` row's `source_summary` + `extracted_data` as context) and `reextract_messages` (concatenates the full message chain — `primary_message_id` + `related_message_ids` — in chronological order). Both delegate to `runLlmReextract`, which reads the `schema_evolutions.diff` as `{ kind: 'llm_field', target_column, extract_prompt, confidence_threshold? }`, calls `deps.llmClient.infer`, parses the LLM response as `{ value, confidence }` JSON, and writes the column with outcome bands: `>= confidence_threshold` → `wrote`; `>= 0.3` → `low_confidence` (still writes); `< 0.3` → `failed` (no write). Plugin entry registers both strategies at boot.
