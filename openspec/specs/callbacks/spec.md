...
`callbacks` is the Telegram inline-keyboard handler for the `strata` namespace. When a user taps `✅ 记录`, `✏️ 调整`, or `❌ 不记` under a pending-event confirmation message, the SDK routes the click to `handleStrataCallback(deps)`; the handler dispatches the matching state transition (`commit` reuses `commitEventCore` from `event-tools`; `abandon` writes `abandoned_reason='user_declined_via_inline'`; `edit` clears the keyboard and waits for a follow-up message) and then edits the original message so the UI converges to a final state. `buildStrataKeyboard(eventId)` is co-located so any future code path that *sends* the keyboard uses the same `callback_data` strings the handler expects.

The capability ships the handler only — actually delivering the keyboard to the user requires an outbound surface the OpenClaw SDK does not yet expose to plugins. The handler is ready the moment that surface lands.
