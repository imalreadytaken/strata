# pending-buffer Specification

## Purpose

`pending-buffer` is the runtime mirror of `raw_events.status = 'pending'`: a per-session set of event ids the agent might still extend ("actually it was ¥48") or that need to be timed-out cleanly. The in-memory `Map<session, Set<event_id>>` keeps the hot path O(1); a JSON state file at `<dataDir>/.strata-state/pending_buffer.json` is written after every mutation so a crash or restart picks up where it left off. A background `setInterval` drains stale pending rows after `config.pending.timeoutMinutes` — high-confidence rows (≥ 0.7) auto-commit; everything else gets `status='abandoned'` with `abandoned_reason='pending_timeout'`. Persistence failures are best-effort: logged at warn, never propagated to the caller.
