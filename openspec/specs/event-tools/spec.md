...
The system SHALL expose `registerEventTools(api: OpenClawPluginApi, runtime: StrataRuntime): void` that calls `api.registerTool(factory)` six times — once per tool. Each factory closes over `ctx.sessionId` (defaulting to `'default'` with a `warn` log when missing) and the runtime's `rawEventsRepo`, `pendingBuffer`, `logger`. The plugin's `register(api)` invokes `registerEventTools` after `startPendingTimeoutLoop`.

#### Scenario: Plugin entry registers all six tools

- **WHEN** the plugin's `register(api)` runs with a stub `api` that records `registerTool` calls
- **THEN** `registerTool` has been called exactly six times with factories whose returned tools have names `strata_create_pending_event`, `strata_update_pending_event`, `strata_commit_event`, `strata_supersede_event`, `strata_abandon_event`, `strata_search_events`
