...
`triage` is Strata's intent classifier — given a user message plus session context, decide whether the agent should run the `capture`, `query`, `build_request`, `correction`, or `chitchat` flow. The classifier is a pure function (`classifyIntent`) over a thin `LLMClient` seam, so it is testable without an LLM and swappable when a real OpenClaw inference path lands. Today it ships with `HeuristicLLMClient` — a deterministic ordered-regex backend that correctly handles the spec's worked examples and defaults to `chitchat` whenever no rule fires (the spec's "prefer chitchat over capture when uncertain" directive). The classifier is exposed but not yet invoked by a `message_received` / `before_agent_start` hook; that wiring is a P5 concern.
