...
The system SHALL expose `registerEventTools(api, runtime): void` that calls `api.registerTool(factory)` once. The factory closes over `ctx.sessionId` (defaulting to `'default'`) and the runtime's repos, pending buffer, logger, plus `pipelineDeps` and `buildDeps` bundles. The factory returns NINE tools:
...
#### Scenario: Plugin entry registers all nine tools

- **WHEN** the plugin's `register(api)` runs with a stub `api` that records `registerTool` calls
- **THEN** `registerTool` is called exactly once with a factory whose returned tools have the nine names above
