# reflect-detectors Specification

## Purpose

`reflect-detectors` is the signal-detection layer of the Reflect Agent. Three pure DB-read functions produce typed signals: `detectNewCapabilityEmergence` (committed events with no active capability accumulate by `event_type` and exceed size + duration thresholds), `detectSchemaEvolutionNeed` (TEXT columns in active capability tables exhibit a dominant value past the skew threshold), and `detectArchiveCandidates` (active capabilities whose `last_write_at` / `last_read_at` are both stale). Each signal carries a `signal_strength` in `[0, 1]` so the proposal generator (next change) can rank. Thresholds live in `REFLECT_THRESHOLDS` and can be overridden per call via `opts.thresholds`. The emergence detector optionally upgrades its `suggested_name` via the runtime `llmClient` when `opts.useLLM === true`; LLM failure falls back to a slugified `event_type`. No cron, no proposal-row writes — those are the next two changes' jobs.
