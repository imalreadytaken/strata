# reflect-cron Specification

## Purpose

`reflect-cron` is the operational top of the Reflect Agent. `runReflectOnce(deps)` is the composable one-shot — it runs every detector, generates proposals, and optionally pushes via a caller-supplied `notify`. `startReflectAgent(deps, opts)` is the scheduler: an interval-driven check that fires `runReflectOnce` once per week at the configured `dayOfWeek + hour` (default Sunday 03:00), guarded against double-fire by an `alreadyFiredThisWeek` check against the proposals table itself. `handleReflectCallback(deps)` is the Telegram interactive handler registered under namespace `strata-propose`; it parses `approve:<id>` / `decline:<id>` payloads, flips the proposal status (decline sets `cooldown_until = now + 30 days`), and edits the original message to clear buttons. Plugin `register(api)` wires both the cron and the callback handler, and stows the cron's `stop` handle on `runtime.stopReflect`.
