# query-skill Specification

## Purpose

`query-skill` is the read-only path off the triage router: when a user message asks a "history / aggregate / top-N" question (e.g. "how much did I spend on dining last month?"), triage routes to the `query` skill which knows to call `strata_query_table` for structured business-table queries and `strata_search_events` for raw-event lookups. The tool `strata_query_table` reads `capability_registry.primary_table`, validates every column reference against `PRAGMA table_info` (no string concat — values bind through `?` placeholders), and answers either as a row list or as a `{fn, column, value}` aggregate. `since` / `until` always bind to `occurred_at` (the AGENTS.md-mandated time column); `limit` is hard-capped at 100. The agent flow is documented in `src/skills/query/SKILL.md` with three worked examples (money aggregation, count-over-time, top-N) plus an explicit ban on calling `strata_create_pending_event` from inside a query flow.
