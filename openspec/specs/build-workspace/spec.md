# build-workspace Specification

## Purpose

`build-workspace` is the per-build workdir scaffolder. `setupBuildWorkspace` creates `<buildsDir>/<sessionId>-<timestamp>/` and materialises everything Claude Code reads at startup: a verbatim copy of the constitution (`AGENTS.md`), the approved plan (`PLAN.md`), a freshly-rendered `USER_CONTEXT.md` reflecting the live capability registry and pending proposals, and a `existing_capabilities/<name>/` snapshot containing `meta.json` + `migrations/` for each active capability. It then runs `git init` + an initial commit so the integration phase has a stable rollback target. `renderUserContext` is a pure async helper exported separately so tests can pin its output. `cleanupBuildWorkspace` is the symmetric tear-down — idempotent so stop/resume flows can call it freely.

The capability pairs with `claude-code-runner`: the runner's `opts.workdir` is exactly the path this helper returns.
